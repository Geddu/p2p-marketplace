        ...
        if (error) {
          ...
          for (const item of data || []) {
            if (item.images && item.images.length > 0) {
              const url = await getAuthenticatedImageUrl(item.images[0])
              if (url) {
                urls[item.id] = url
              }
            }
          ...
          <CardContent className="p-0">
            <div className="flex flex-col sm:flex-row items-center">
              <div className="w-full sm:w-24 h-48 sm:h-24 relative">
                <Image
                  ...
                  onError={(e) => {
                    ...
                    console.error(`Failed to load image for item ${item.id}:`, item.images[0])
                  }}
                />
              </div>
              <div className="p-4 flex-grow w-full">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center text-sm text-muted-foreground mt-2">
                  ...
                  <div className="flex space-x-3">
                    ...
                    <span className="flex items-center">
                      <MessageCircle className="w-4 h-4 mr-1" /> {item.conversations}
                    </span>
                  </div>
                </div>
              </div>
              <div className="p-4 flex justify-center sm:justify-end space-x-2 w-full sm:w-auto">
                ...
                <Button variant="outline" size="icon" className="rounded-full w-10 h-10" onClick={() => openChat(item)}>
